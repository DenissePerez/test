...
<a class="active">Confirmar {{ view.action_name|title }}</a>
{% endblock %}
        ...
        <div class="card-content">
            <table class="table selectable bordered">
                ...
                <tbody>
                    ...
                    {% empty %}
                    <tr><td colspan="4">No ha seleccionado ninguna tarea</td></tr>
                    {% endfor %}
                </tbody>
            </table>
        </div>
        <div class="card-action">
            <div class="right-align">
                <form method="POST">
                    {% csrf_token %}
                    {{ form }}
                    <button  type="submit" name="_confirm" class="btn primary white-text btn-flat">{{ view.action_name }}</button>
                </form>
            </div>
        </div>
    </div>
